...
function saveAllData() {
  localStorage.setItem('exerciseList', JSON.stringify(exercisesList)); // ذخیره لیست حرکات
  alert("تمام حرکات ذخیره شد.");
}

function loadAllData() {
  const savedExercises = localStorage.getItem('exerciseList');
  if (savedExercises) {
    exercisesList.length = 0; // پاک کردن لیست فعلی حرکات
    exercisesList.push(...JSON.parse(savedExercises)); // بارگذاری حرکات از Local Storage
    alert("تمام حرکات بارگذاری شد.");
  } else {
    alert("هیچ حرکتی برای بارگذاری موجود نیست.");
  }
}
